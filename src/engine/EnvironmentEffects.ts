 ...
 * - 네온 조명 pulse 애니메이션
 *
 * 모든 파티클은 Points + BufferGeometry 기반 GPU 친화적 렌더링
 */
...
export class EnvironmentEffects {
  ...
  constructor(scene: THREE.Scene, config: EnvironmentEffectsConfig = {}) {
    ...
    this.dustPool = new ParticlePool(
      DUST_MAX_PARTICLES, DUST_SIZE, DUST_COLOR, THREE.NormalBlending,
    );
  ...
  emitHitEffect(hitPoint: THREE.Vector3, hitNormal?: THREE.Vector3): void {
    ...
    for (let i = 0; i < DUST_COUNT_PER_HIT; i++) {
      ...
      this.dustPool.emit(
        hitPoint,
        dir.multiplyScalar(DUST_SPEED * (0.3 + Math.random() * 0.7)),
        DUST_LIFETIME * (0.5 + Math.random() * 0.5),
      );
    }
  }
