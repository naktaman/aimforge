...
- 시나리오 20종 코드 존재 (실동작 미검증)
- 캘리브레이션 트라이얼 루프 코드 배선됨 (실동작 미검증)
- DNA 배터리 코드 배선됨 (실동작 미검증)
...
### B-2: 총기 시스템 고도화 [딥리서치 필요]
**현재**: WeaponViewModel 418줄 (프로시저럴 권총/라이플 지오메트리), WeaponSystem 310줄
**목표**: 시각적으로 설득력 있는 1인칭 무기

리서치 항목:
- FPS 게임 뷰모델 렌더링 기법 (separate render pass, FOV)
- 머즐 플래시 파티클 시스템 설계
- 반동 패턴 시각화 (CS2 스프레이 패턴 vs Valorant 첫탄 반동)
...
### C-1: GP 캘리브레이션 완전 동작
**현재**: 코드 배선됨 (useCalibrationHandlers), 실동작 미검증
- [ ] 활성 프로필 기반 캘리브레이션 시작
- [ ] 트라이얼 루프: get_next_trial_sens → 시나리오 → submit → 반복
- [ ] GP 곡선 + 관측점 실시간 표시 (애니메이션: 점이 찍히며 곡선 피팅)
- [ ] 수렴 감지 → finalize → 결과 화면 (바 올라가며 숫자 카운트업)
- [ ] "감도 적용" → settingsStore + GameProfile DB 동시 업데이트
- [ ] 줌 캘리브레이션 동작 확인

### C-2: Aim DNA 배터리 완전 동작
...
### C-3: 시나리오 시스템 점검 [딥리서치 필요]
**현재**: 20종 시나리오 코드 존재, 실동작 미검증

리서치 항목:
- Aim Lab/Kovaak's 시나리오 설계 원칙
- 타겟 스폰 패턴 알고리즘 (균일분포 vs 적응형)
- 난이도 스케일링 커브 이론
...
**다음:**
1. **B-2~B-4 인게임 비주얼** (딥리서치 → 구현)
2. **C-1 캘리브레이션 end-to-end**
3. **C-2 DNA 배터리 end-to-end**
4. **C-3 시나리오 시스템 점검** (딥리서치 → 구현)
5. **C-4 트레이닝 시스템**
6. **A-1 설정 나머지 섹션** (디스플레이, 크로스헤어, 사운드, HUD, 데이터, 단축키)
7. **B-1 사운드 Phase 2** (리버브, 앰비언트, 스폰 사운드)
8. **E-1/E-2 인사이트 + 크로스게임**
